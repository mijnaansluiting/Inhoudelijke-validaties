<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <xsl:variable name="extra_object_names" select="$found_object_names[not(some $object_name in $expected_object_names satisfies . = $object_name)]"/>
      <xsl:variable name="is_correctly_covered" select="(empty($missing_object_names) and empty($extra_object_names)) or ($expected_object_names = ('AlleObjecten', 'Document'))"/>      
      ...
      <xsl:template match="/">
            <xsl:value-of select="keronic:as-markdown-row($rule_number, $expected_object_names, $found_object_names, $missing_object_names, $extra_object_names)"/>
            <xsl:value-of select="','"/>
